...
namespace Wx.App.BaiduMapGo.Common
{
    ...
    public class BaiduProjection
    {
        ...
        private static readonly int[] _llband =
        {
            75, 60, 45, 30, 15, 0
        };
        ...
        private static readonly double[][] _mc2Ll =
        {
            ...
            new[]
            {
                3.09191371068437e-9, 0.000008983055096812155, 0.00006995724062, 23.10934304144901, -0.00023663490511, -0.6321817810242, -0.00663494467273, 0.03430082397953, -0.00466043876332,
                2555164.4
            },
            new[]
            {
        ...
        private static readonly double[] _mcband =
        {
        ...
        public MercatorPoint CoordinateToMercator(double latitude, double longitude)
        {
            double[] mc = null;

            var lat = Math.Min(Math.Max(latitude, MinLatitude), MaxLatitude);
            var lng = Math.Min(Math.Max(longitude, MinLongitude), MaxLongitude);
        ...
        //    var location = Convertor(p.X, p.Y, mc);
        ...
        private double[] Convertor(double x, double y, double[] mc)
        {
            ...
            var ny = mc[2] + (mc[3] * c) + (mc[4] * c * c) + (mc[5] * c * c * c) + (mc[6] * c * c * c * c) + (mc[7] * c * c * c * c * c) + (mc[8] * c * c * c * c * c * c);
            if (x < 0)
            {
                nx *= -1;
            }
